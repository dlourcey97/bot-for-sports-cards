...
- **Root package.json scripts**: Use `npm run lint` for type checking and `npm run test` for unit tests from the repo root. These are the canonical entry points.
- **Dry run timing**: `dry-run` takes ~50 seconds because it launches a real headless Chromium instance. The `seed` command must be run first to create demo data.
